...
const getData = async () => {
    const url = `${process.env.API_URL}/tag-list/`
...
const Tags = ({data}) => {
    ...
    return(
        <>
            <Head>
                <title>{`Codes List - ${data.site_title} | ${data.site_tagline}`}</title>
                <meta content={data.site_desc} name="description"/>
                <meta name="og:title" content={`Codes List - ${data.site_title} | ${data.site_tagline}`}/>
            ...
            <div className="w-auto pt-5">
                <div className="container my-6 mx-auto px-4 md:px-12">
                    <div className="flex">
                        <h2 className="text-bold text-3xl pb-3 border-b-4 border-solid mb-10">Codes List
                        </h2>
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-4" id="taxlist">
                        {data.itemLists.map(tag => {
                            return (<>
                                <Link href={`/archive/tag/${tag.slug}`} alt={tag.name} className="hover:rounded-lg hover:bg-black/20 px-4 py-2">
