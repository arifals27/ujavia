...
const getData = async () => {
    const url = `${process.env.API_URL}/label-list/`
...
const Labels = ({data}) => {
    ...
    return(
        <>
            <Head>
                <title>{`Labels List - ${data.site_title} | ${data.site_tagline}`}</title>
                <meta content={data.site_desc} name="description"/>
                <meta name="og:title" content={`Labels List - ${data.site_title} | ${data.site_tagline}`}/>
            ...
            <div className="w-auto pt-5">
                <div className="container my-6 mx-auto px-4 md:px-12">
                    <div className="flex">
                        <h2 className="text-bold text-3xl pb-3 border-b-4 border-solid mb-10">Labels List
                        </h2>
                    </div>
